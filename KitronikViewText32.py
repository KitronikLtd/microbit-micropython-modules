...
class Text32:
    ...
    def clearScreen(self):
        ...
        self.displayStringOnLine(self.LCD_LINE1, '')
        self.displayStringOnLine(self.LCD_LINE2, '')
    ...
    def showString(self, text):
        ...
        if numberOfStrings <= 2:
            self.displayStringOnLine(self.LCD_LINE1, displayStrings[0])
            if numberOfStrings == 1:
                self.displayStringOnLine(self.LCD_LINE2, '')
            else:
                self.displayStringOnLine(self.LCD_LINE2, displayStrings[1])
        else:
            #loop around will all of the strings have been displayed
            for textLoop in range(numberOfStrings - 1):
                self.displayStringOnLine(self.LCD_LINE1, displayStrings[textLoop])
                self.displayStringOnLine(self.LCD_LINE2, displayStrings[textLoop + 1])
                sleep(2000)

    def scroll(self, displayLine, text):
        textScroll = 0
        lengthOfText = len(text)

        if lengthOfText < 16:
            while (lengthOfText < 16):
                text = text + " "
                lengthOfText = len(text)
        
        if displayLine is "Top":
            address = self.LCD_LINE1
        elif displayLine is "Bottom":
            address = self.LCD_LINE2

        for textScroll in range (0, (lengthOfText - 15)):
            self.displayStringOnLine(address, text[textScroll:(textScroll+16)])
            sleep(500)